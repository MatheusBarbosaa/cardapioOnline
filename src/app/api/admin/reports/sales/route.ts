...
export async function GET(request: NextRequest) {
  try {
    ...
    const now = new Date();
    const defaultStartDate = new Date(now.getFullYear(), now.getMonth() - 11, 1); // Últimos 12 meses
    const defaultEndDate = new Date(now.getFullYear(), now.getMonth() + 1, 0); // Final do mês atual

    const queryStartDate = startDate ? new Date(startDate) : defaultStartDate;
    const queryEndDate = endDate ? new Date(endDate) : defaultEndDate;

    // Buscar pedidos com status de pagamento confirmado ou finalizado
...
function processSalesDataByPeriod(orders: any[], period: string, startDate: Date, endDate: Date) {
  ...
  orders.forEach(order => {
    ...
    data.vendas += order.total;
    data.pedidos += 1;
    data.clientes.add(order.customerCpf); // Usando CPF como identificador único
  });
  ...
  const result = Array.from(groupedData.values())
    .map(item => ({
      ...
      data: undefined // Remove o objeto Date
    }))
...
function calculateKPIs(orders: any[]) {
  ...
  const clientesUnicos = new Set(orders.map(order => order.customerCpf)).size;

  // Calcular crescimento (comparar com período anterior)
  // Aqui você pode implementar lógica mais complexa para calcular crescimento real
  const crescimento = 12.5; // Placeholder - implementar cálculo real

  // Taxa de conversão (placeholder - você pode calcular baseado em visualizações vs compras)
...
function getTopProducts(orders: any[]) {
  ...
  orders.forEach(order => {
    order.orderProducts.forEach((op: any) => {
      ...
      stats.vendas += op.price * op.quantity;
      stats.unidades += op.quantity;
    });
  });
...
function getSalesByCategory(orders: any[]) {
  ...
  orders.forEach(order => {
    order.orderProducts.forEach((op: any) => {
      ...
      const sales = op.price * op.quantity;
  ...
  const result = Array.from(categoryStats.entries())
    .map(([name, value]) => ({
      name,
      value: Math.round((value / totalSales) * 1000) / 10, // Percentual com 1 decimal
